...
class Html
{
    ...
    protected static function parseNode($node, $element, $styles = array(), $data = array())
    {
        ...
        if (isset($nodes[$node->nodeName])) {
            ...
            $method = "parse{$method}";
            $newElement = call_user_func_array(array('App\Utils\Html', $method), array_values($arguments));
    ...
    protected static function parseList($node, $element, &$styles, &$data)
    {
        ...
        if (isset($data['listdepth'])) {
            ...
            $style = $php_word->addNumberingStyle($styles['list'], self::getListStyle($isOrderedList));
