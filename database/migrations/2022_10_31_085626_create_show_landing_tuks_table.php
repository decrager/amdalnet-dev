...
class CreateShowLandingTuksTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    {
        return <<<SQL
            create view ShowLandingTuk as
            select count(tbl.id_luk_member), tbl.authority from (
            select lm.name, lm.id as id_luk_member,
            case 
                when ftt.authority = 'Provinsi' then p.name
                when ftt.authority = 'Kabupaten/Kota' then d.name
                else ftt.authority
            end authority
            from luk_members lm 
    ...
    {
        return <<<SQL
            drop view if exists `ShowLandingTuk`;
            SQL;
    }
}
